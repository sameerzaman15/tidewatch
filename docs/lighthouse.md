...
Mobile preset against a production build (`pnpm build && pnpm start`) for `/`. Chrome in this environment, Lighthouse 12, simulated slow 4G. Measured on 25 Sep 2026.
...
- CLS: 0 (under 0.05).
- LCP: 2.4 s. FCP: 1.5 s. Total blocking time: 50 ms.
- First-load JS for `/`: **187.3 KB gzipped**. The aim was under 150 KB. React DOM and the Next.js runtime are most of that number, so the page does not reach the aim.
